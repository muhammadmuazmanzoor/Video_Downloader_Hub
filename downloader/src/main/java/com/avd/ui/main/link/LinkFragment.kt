...
import com.avd.databinding.FragmentLinkBinding
import com.avd.ui.component.dialog.DownloadDialogListener
import com.avd.ui.component.dialog.showDownloadVideoDialog
...
class LinkFragment : BaseFragment(), IDownloadInfoLinkListener {
    ...
    private  val linkViewModel: DownloadLinkViewModel by viewModels()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        linkViewModel.showDownloadDialogEvent.observe(viewLifecycleOwner) { videoInfo ->
            ...
            activity?.runOnUiThread {
                DownloaderModuleNavigator.mainViewModel?.let {
                    showDownloadVideoDialog(
                        ...
                        object : DownloadDialogListener {
                            ...
                            override fun onSelectFormat(videoInfo: VideoInfo, format: String) {
                                val currentFormat = DownloaderModuleNavigator.mainViewModel?.selectedFormatTitle
                                DownloaderModuleNavigator.mainViewModel?.selectedFormatTitle?.set(currentFormat?.get()
                                    ?.let { Pair(format, it.second) })
                            }
                        }, it, videoInfo, appUtil
                    )
                }
            }
        }
