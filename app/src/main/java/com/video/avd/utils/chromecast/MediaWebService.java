...
import com.video.avd.utils.chromecast.webserver.SimpleWebServer;
...
public class MediaWebService extends IntentService {
    ...
    public int onStartCommand(@Nullable Intent intent, int flags, int startId) {
        if (intent != null) {
            mPendingIntent = intent.getParcelableExtra(CastConstant.PENDING_INTENT_SERVICE);
        } else {
            sendErrorMessage();
            stopSelf();
        }
    ...
    protected void onHandleIntent(@Nullable Intent intent) {
        ...
        if (intent.getExtras() == null || intent.getExtras().getString(CastConstant.IP_LINK_KEY) == null) {
            onDestroy();
            return;
        }

        setDeviceIpAddress(intent.getExtras().getString(CastConstant.IP_LINK_KEY));
    ...
    private void sendSuccessMessage() {
        ...
        try {
            mPendingIntent.send(MediaWebService.this, CastConstant.CONNECT_SUCCESS_MESSAGE, result);
    ...
    private void sendErrorMessage() {
        Intent result = new Intent();
        try {
            mPendingIntent.send(MediaWebService.this, CastConstant.CONNECT_ERROR_MESSAGE, result);
        } catch (PendingIntent.CanceledException e) {
            e.printStackTrace();
        }
    }
    ...
    private void sendDestroyMessage() {
        ...
        try {
            if (mPendingIntent != null){
                mPendingIntent.send(MediaWebService.this, CastConstant.CONNECT_DESTROY_MESSAGE, result);
            }
        } catch (PendingIntent.CanceledException e) {
            e.printStackTrace();
        } catch (Exception e){
