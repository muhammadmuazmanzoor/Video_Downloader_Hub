...
object HlsPlaylistParser {
    ...
    fun parse(playlistContent: String, baseUri: String): HlsPlaylist {
        try {
            BrowserKitLog.d("HlsParser", "HLS Parser: Attempting to parse with ExoPlayer parser. manifest body length: ${playlistContent.length}")
            return parseWithExoPlayer(playlistContent, baseUri)
        } catch (exoPlayerError: Exception) {
            BrowserKitLog.w("HlsParser", "HLS Parser: ExoPlayer parser failed ('${exoPlayerError.message}'). Falling back to legacy parser.")
            try {
