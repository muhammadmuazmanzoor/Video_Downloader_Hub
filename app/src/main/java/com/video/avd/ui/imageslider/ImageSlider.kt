package com.video.avd.ui.imageslider
...
import com.video.avd.ui.imageslider.adapters.ViewPagerAdapter
import com.video.avd.ui.imageslider.animations.BackgroundToForeground
import com.video.avd.ui.imageslider.animations.CubeIn
import com.video.avd.ui.imageslider.animations.CubeOut
import com.video.avd.ui.imageslider.animations.DepthSlide
import com.video.avd.ui.imageslider.animations.FidgetSpinner
import com.video.avd.ui.imageslider.animations.FlipHorizontal
import com.video.avd.ui.imageslider.animations.FlipVertical
import com.video.avd.ui.imageslider.animations.ForegroundToBackground
import com.video.avd.ui.imageslider.animations.Gate
import com.video.avd.ui.imageslider.animations.RotateDown
import com.video.avd.ui.imageslider.animations.RotateUp
import com.video.avd.ui.imageslider.animations.Toss
import com.video.avd.ui.imageslider.animations.ZoomIn
import com.video.avd.ui.imageslider.animations.ZoomOut
import com.video.avd.ui.imageslider.constants.AnimationTypes
import com.video.avd.ui.imageslider.constants.ScaleTypes
import com.video.avd.ui.imageslider.interfaces.ItemChangeListener
import com.video.avd.ui.imageslider.interfaces.ItemClickListener
import com.video.avd.ui.imageslider.interfaces.TouchListener
import com.video.avd.ui.imageslider.models.SlideModel
...
class ImageSlider @JvmOverloads constructor(
    ...
    private var belowViewPager: Boolean = true  // Default value
    ...
    init {
        ...
        context.theme.obtainStyledAttributes(
            ...
            try {
                belowViewPager = getBoolean(R.styleable.ImageSlider_dotsBelowViewPager, true)
            } finally {
                recycle()
            }
        }
        ...
        configureDotsLayout(belowViewPager)
    ...
    /**
     * Configures the layout of the pager dots relative to the view pager.
     * @param belowViewPager Boolean indicating whether the dots should be below the view pager.
     */
    private fun configureDotsLayout(belowViewPager: Boolean) {
        val viewPagerParams = viewPager?.layoutParams as RelativeLayout.LayoutParams

        if (belowViewPager) {
            viewPagerParams.addRule(RelativeLayout.ABOVE, R.id.pager_dots)
            viewPagerParams.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM, 0) // Remove align bottom rule if exists
            pagerDots?.setPadding(0, 0, 0, 0) // Reset padding
            val marginTop = resources.getDimensionPixelSize(R.dimen.dots_margin_top)
            val dotsParams = pagerDots?.layoutParams as RelativeLayout.LayoutParams
            dotsParams.topMargin = marginTop
            pagerDots?.layoutParams = dotsParams
        } else {
            viewPagerParams.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM)
            viewPagerParams.addRule(RelativeLayout.ABOVE, 0) // Remove the above rule
            val paddingBottom = resources.getDimensionPixelSize(R.dimen.view_pager_padding_bottom)
            viewPager?.setPadding(0, 0, 0, paddingBottom)
        }

        viewPager?.layoutParams = viewPagerParams // Reapply layout params to take effect
    }
