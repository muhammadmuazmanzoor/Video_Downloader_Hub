...
class VideoDetectionAlgVModel @Inject constructor(
    ...
    var downloadButtonState = ObservableField<DownloadButtonState>(DownloadButtonStateCanNotDownload())
    ...
    override fun verifyLinkStatus(resourceRequest: Request, hlsTitle: String?) {
        ...
        if (lastVerifiedLink != urlToVerify) {
            ...
            if (urlToVerify.contains(".m3u8") || urlToVerify.contains(".mpd") || (urlToVerify.contains(
                ...
                if (::settingsModel.isInitialized){
    ...
    private fun startVerifyProcess(
        ...
        verifyVideoLinkJobStorage[resourceRequest.url.toString()] = io.reactivex.rxjava3.core.Observable.create { emitter ->
                    ...
                    if (info.id.isNotEmpty()) {
                        ...
                        if (state is DownloadButtonStateCanDownload) {
                            if (state.info?.isRegularDownload == true) {
                                AppLogger.d("Watching set new info state with Regular Download... currentState: $state skippingInfo: $info")
                            }
                        }
                        if (state is DownloadButtonStateCanDownload && state.info?.isM3u8 == true && state.info.isMaster && isLastNotEmpty || (state is DownloadButtonStateCanDownload && state.info?.isRegularDownload != true && info.isRegularDownload) || state is DownloadButtonStateLoading && info.isRegularDownload) {
                            AppLogger.d("Skipping set new info state... currentState: $state skippingInfo: $info")
                        } else {
                            AppLogger.d("Setting set new info state... state: $state info: $info")
