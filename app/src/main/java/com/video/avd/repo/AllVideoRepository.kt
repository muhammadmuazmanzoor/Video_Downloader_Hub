...
@Singleton
class AllVideoRepository(val local:AppDatabase,val repository: Repository) {
    ...
    fun deleteSongFromDb(track: Video) = CoroutineScope(Dispatchers.IO).launch {
        if (repository.isObjectExists(track.title.toString())) {
            repository.localRepo.dao().deleteSongByName(track.title.toString())
        } else {
