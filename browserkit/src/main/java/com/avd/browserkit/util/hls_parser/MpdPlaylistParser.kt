package com.avd.browserkit.util.hls_parser
...
object MpdPlaylistParser {
    ...
    private fun translateRepresentation(
        ...
        if (index == null) {
            BrowserKitLog.d("MpdParser", "MPD Parser: No segment index found for representation ${exoRep.format.id}.")
        } else {
            if (isLive) {
                // Case 1: LIVE stream logic (as determined by the top-level manifest)
                BrowserKitLog.d("MpdParser", "MPD Parser: Detected LIVE index for ${exoRep.format.id}.")
                try {
                    val liveEdgeUs =
                    ...
                    if (segmentNum > 0) {
                        ...
                        BrowserKitLog.d("MpdParser", "MPD Parser (Live): Calculated live edge segment #$segmentNum for ${exoRep.format.id}")
                    } else {
                        BrowserKitLog.w("MpdParser", "MPD Parser (Live): Calculated a non-positive segment number ($segmentNum), skipping.")
                    }
                } catch (e: Throwable) {
                    BrowserKitLog.e("MpdParser", "MPD Parser (Live): Failed to calculate live edge segment ${e.message}")
                    e.printStackTrace()
                }
            } else {
                // Case 2: VOD (Static) stream logic
                BrowserKitLog.d("MpdParser", "MPD Parser: Detected VOD index for ${exoRep.format.id}.")
                if (periodDuration != null) {
                    ...
                    if (segmentCount > 0) {
                        ...
                        BrowserKitLog.d("MpdParser", "MPD Parser (VOD): Found $segmentCount segments for ${exoRep.format.id}.")
                    } else {
                        BrowserKitLog.d("MpdParser", "MPD Parser (VOD): No segments found for ${exoRep.format.id} (count: $segmentCount).")
                    }
                }
            }
        }
        ...
        if (drmInitData != null) {
            ...
            for (i in 0 until drmInitData.schemeDataCount) {
                ...
                BrowserKitLog.d("MpdParser", "MPD Parser: Found DRM scheme ${schemeData.uuid} for rep ${exoRep.format.id}")
            }
        }
