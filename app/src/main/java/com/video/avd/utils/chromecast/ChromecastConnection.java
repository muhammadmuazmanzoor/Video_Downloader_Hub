...
public class ChromecastConnection {
    ...
    public static List<Video> listofvideos=new ArrayList<Video>();
    ...
    /** Cached CastContext when pre-initialized off main thread to avoid ANR from Binder in getSharedInstance. */
    ...
    /**
     * Constructor that accepts a pre-initialized CastContext to avoid calling
     * CastContext.getSharedInstance() on the main thread (which triggers a Binder call and can cause ANR).
     * Use this when initializing Chromecast from an Activity by loading CastContext on a background thread first.
     */
    public ChromecastConnection(Activity activity, CastStateUpdateListener connectionCastStateUpdateListener, CastContext castContext) {
        this.mActivity = activity;
        this.mCastStateUpdateListener = connectionCastStateUpdateListener;
        this.mCastContext = castContext;

        if (castContext != null) {
            castContext.addCastStateListener(mCastStateUpdateListener);
            mCastStateUpdateListener.onCastStateChanged(castContext.getCastState());
        }
    }
        ...
        mActivity.runOnUiThread(() -> {
            ...
            if (session == null) {
                try {
                    ...
                    MediaRouteSelector mediaRouteSelector = new MediaRouteSelector.Builder().addControlCategory(CastMediaControlIntent.categoryForCast(mAppId)).build();
                    MediaRouter mediaRouter = MediaRouter.getInstance(mActivity);
                    MediaRouter.Callback mediaRouterCallback = new MediaRouter.Callback() {
                        @Override
                        public void onRouteSelected(MediaRouter router, MediaRouter.RouteInfo route) {
                            super.onRouteSelected(router, route);
                            // Handle the route selection
                            callback.onItemClick();
                        }
                        // Implement other callback methods if needed
                    };
                    mediaRouter.addCallback(mediaRouteSelector, mediaRouterCallback, MediaRouter.CALLBACK_FLAG_REQUEST_DISCOVERY);
                    MediaRouteChooserDialog builder = new MediaRouteChooserDialog(mActivity, R.style.CustomMediaRouteDialogTheme);
//                    builder.getWindow().setBackgroundDrawableResource(android.R.color.white);
                    builder.setRouteSelector(mediaRouteSelector);
    ...
    public void requestStartSessionFromHome(RequestSessionCallbackFromHome callback) {
        mActivity.runOnUiThread(() -> {
            CastSession session = getSession();
            if (session == null) {
                try {
                    ...
                    MediaRouteChooserDialog builder = new MediaRouteChooserDialog(mActivity, R.style.CustomMediaRouteChooserDialogTheme);
                    builder.setRouteSelector(mediaRouteSelector);
                    builder.setCanceledOnTouchOutside(true);
                    builder.setOnCancelListener(dialog -> {
                        mediaRouter.removeCallback(mediaRouterCallback);
...
//    }
    ...
    public abstract static class RequestSessionCallbackFromHome implements ConnectionCallback {
        public abstract void onError(int errorCode);
