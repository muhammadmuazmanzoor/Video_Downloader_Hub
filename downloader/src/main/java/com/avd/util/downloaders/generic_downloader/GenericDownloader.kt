package com.avd.util.downloaders.generic_downloader
...
//        }
         fun compressString(text: String): String {
            ...
            val buffer = ByteArray(1024) // Using a larger buffer for better performance
...
//            val compressedData = Base64.getDecoder().decode(compressedText)
            // Assuming compressedText is your Base64 encoded string
            val compressedData = try {
                Base64.decode(compressedText, Base64.DEFAULT) // Decode the Base64 string
            } catch (e: IllegalArgumentException) {
                // Handle potential errors during decoding
                e.printStackTrace()
                ByteArray(0) // Return an empty byte array in case of error
            }
