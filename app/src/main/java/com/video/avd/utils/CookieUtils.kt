...
//        "${ContextUtils.getApplicationContext().filesDir.parentFile}/app_webview/Default/"
    val chromeDefaultPathApi28Less by lazy {
        "${ContextUtils.getApplicationContext().filesDir.parentFile}/app_webview/"
    }
    val chromeDefaultPathApi29More by lazy {
        "${ContextUtils.getApplicationContext().filesDir.parentFile}/app_webview/Default/"
    }
    ...
    init {
        try {
            Log.d("CookieUtils", "Initializing CookieUtils")
            // other initialization logic
        } catch (e: Exception) {
            Log.e("CookieUtils", "Initialization error", e)
            throw e
        }
    }
