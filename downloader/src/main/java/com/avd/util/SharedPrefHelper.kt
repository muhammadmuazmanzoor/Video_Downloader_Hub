...
class SharedPrefHelper @Inject constructor(context: Context) {
    companion object {
        ...
        private const val DOWNLOAD_DIALOG_SHOWN = "DOWNLOAD_DIALOG_SHOWN"
        private const val DOWNLOAD_DIALOG_SESSION_ID = "DOWNLOAD_DIALOG_SESSION_ID"
        private const val TIKTOK_FEATURE_DIALOG_SHOWN = "TIKTOK_FEATURE_DIALOG_SHOWN"
        private const val TIKTOK_DOWNLOAD_ENABLED = "TIKTOK_DOWNLOAD_ENABLED"
        private const val OVERLAY_PERMISSION_PENDING = "OVERLAY_PERMISSION_PENDING"
    }
    ...
    fun saveDownloadWifi(value : Boolean){
        sharedPreferences.edit().let {
            it.putBoolean(KEY_WIFI_ONLY, value)
            it.apply()
        }
    }

    fun getDownloadWifi() : Boolean {
        return sharedPreferences.getBoolean(KEY_WIFI_ONLY, false)
    }

    // Download completion dialog tracking methods
    fun isDownloadDialogShown(): Boolean {
        return sharedPreferences.getBoolean(DOWNLOAD_DIALOG_SHOWN, false)
    }

    fun setDownloadDialogShown(isShown: Boolean) {
        sharedPreferences.edit().let {
            it.putBoolean(DOWNLOAD_DIALOG_SHOWN, isShown)
            it.apply()
        }
    }

    fun getDownloadDialogSessionId(): String {
        return sharedPreferences.getString(DOWNLOAD_DIALOG_SESSION_ID, "") ?: ""
    }

    fun setDownloadDialogSessionId(sessionId: String) {
        sharedPreferences.edit().let {
            it.putString(DOWNLOAD_DIALOG_SESSION_ID, sessionId)
            it.apply()
        }
    }
    ...
    fun resetDownloadDialogSession() {
        sharedPreferences.edit().let {
            it.putBoolean(DOWNLOAD_DIALOG_SHOWN, false)
            it.putString(DOWNLOAD_DIALOG_SESSION_ID, "")
            it.apply()
        }
    }

    // TikTok download feature dialog tracking methods
    fun isTiktokFeatureDialogShown(): Boolean {
        return sharedPreferences.getBoolean(TIKTOK_FEATURE_DIALOG_SHOWN, false)
    }

    fun setTiktokFeatureDialogShown(isShown: Boolean) {
        sharedPreferences.edit().let {
            it.putBoolean(TIKTOK_FEATURE_DIALOG_SHOWN, isShown)
            it.apply()
        }
    }

    // TikTok download switch state methods
    fun isTiktokDownloadEnabled(): Boolean {
        return sharedPreferences.getBoolean(TIKTOK_DOWNLOAD_ENABLED, false)
    }

    fun setTiktokDownloadEnabled(enabled: Boolean) {
        sharedPreferences.edit().let {
            it.putBoolean(TIKTOK_DOWNLOAD_ENABLED, enabled)
            it.apply()
        }
    }

    /** True when user was sent to overlay settings; we auto-enable switch only when they return and this is set. */
    fun isOverlayPermissionPending(): Boolean {
        return sharedPreferences.getBoolean(OVERLAY_PERMISSION_PENDING, false)
    }

    fun setOverlayPermissionPending(pending: Boolean) {
        sharedPreferences.edit().let {
            it.putBoolean(OVERLAY_PERMISSION_PENDING, pending)
            it.apply()
        }
    }
}